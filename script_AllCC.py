...
for cc, len_ in zip(concom, concom_size):
    if len_ <= 5:
        ...
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 6))
        model.draw_graph(with_labels=False, method='KMeans', font_color='red', 
                         title=f"Karate Graph with KMeans labels (Modularity {round(model.metric['modularity']['KMeans'],4)})", ax=ax1)
        model.draw_graph(with_labels=False, method='Louvian', font_color='red', 
                         title=f"Karate Graph with Louvian labels (Modularity {round(model.metric['modularity']['Louvian'],4)})", ax=ax2)
        plt.show()
